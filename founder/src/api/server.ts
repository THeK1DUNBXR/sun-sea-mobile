 ...
 * integration mode.
 *
 *  - "mobile": the Sun Sea ERP has the mobile extension installed (field team data available)
 *              (`/api/mobile/*` — routes, visits, sync, receipts, OCR).
 *  - "direct": a plain Sun Sea ERP (e.g. the Railway deployment) — Insights
...
import { KEYS as STORAGE_KEYS } from './client';
...
export async function saveServer(info: ServerInfo) {
  ...
  await AsyncStorage.multiSet([
    [STORAGE_KEYS.serverInfo + '.mode', info.mode ?? 'direct'],
...
export async function getServerMode(): Promise<ServerMode> {
  try {
    const m = await AsyncStorage.getItem(STORAGE_KEYS.serverInfo + '.mode');
